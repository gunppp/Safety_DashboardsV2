...
export function SafetyDashboard() {
  ...
  const renderPanel = (panel: PanelKey, panelScale = 1) => {
    if (panel === 'slogan') {
      return (
        ...
        <Card title="Safety Calendar" icon={<CalendarIcon className="h-5 w-5 text-sky-600" />} tone="sky" panelScale={panelScale}>
          ...
          <div className="grid grid-cols-7 gap-2">
            {gridCells.map((c, idx) => {
              ...
              return (
                <div key={idx} className={`${base} ${cls}`} style={{ minHeight: scaledPx(80, panelScale, 56, 110) }}>
  ...
  const Slot = ({ slot }: { slot: SlotKey }) => {
    ...
    useEffect(() => {
      ...
      const ro = new ResizeObserver((entries) => {
        ...
        if (!rect) return;
        setPanelScale(panelScaleFromSize(rect.width, rect.height));
      });
      ro.observe(el);
